    ...
    for (int i = 0; i < numberOfTicks; i++) {
      ...
      if (currentTime == startDate || currentTime == endDate) {
        ...
        if (index == -1) {
          ...
          bool flag1 = true;
          bool flag2 = true;
          if (dataHumid1[index] == 101) {
            _chartData1.add(DataChart(0, dataHumid2[index], dataTime[index]));
            _chartData2.add(DataChart(0, dataHumid2[index], dataTime[index]));
            flag1 = false;
          }
          if (dataHumid2[index] == 101) {
            _chartData1.add(DataChart(dataHumid1[index], 0, dataTime[index]));
            _chartData2.add(DataChart(dataHumid1[index], 0, dataTime[index]));
            flag2 = false;
          }
          if (flag1 && flag2) {
            _chartData1.add(DataChart(
                dataHumid1[index], dataHumid2[index], dataTime[index]));
            _chartData2.add(DataChart(
                dataHumid1[index], dataHumid2[index], dataTime[index]));
          }
        }
        ...
        if (count >= 3) {
          counter++;
          //print(counter);
          if (count == 3 && dataHumid1[index + 1] == 0) {
          } else if (counter % (sqrt(counter)).toInt() == 0) {
            _chartData1.add(DataChart(0.0, 0.0, currentTime));
            _chartData2.add(DataChart(0.0, 0.0, currentTime));
          }
        } else {
